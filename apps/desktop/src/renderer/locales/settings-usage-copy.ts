...
export type UsageSettingsCopy = {
  ...
  pricing: {
    title: string; description: string; add: string; edit: string; remove: string;
    ...
    removeTitle(modelKey: string): string; removeDescription: string; actionsAria: string; noCache: string;
  };
  tables: {
...
const SETTINGS_USAGE_COPY = {
  zh: {
    ...
    pricing: {
      ...
      add: '添加价格', edit: '编辑', remove: '删除', save: '保存价格', cancel: '取消', saving: '保存中…', removing: '删除中…',
      ...
      removeTitle: (modelKey) => `删除 ${modelKey} 的定价覆盖？`, removeDescription: '删除后该模型将恢复使用内置定价（如果存在）。', actionsAria: '定价覆盖操作', noCache: '未设置',
    },
    tables: {
      ...
      pricingHeaders: ['供应商', '模型', '输入 / 1M', '输出 / 1M', '缓存读取 / 1M', '缓存写入 / 1M', '操作'], requestHeaders: ['时间', '类型', '对象', '会话', 'Token', '费用', '延迟', '状态'],
  ...
  en: {
    ...
    pricing: {
      ...
      add: 'Add pricing', edit: 'Edit', remove: 'Delete', save: 'Save pricing', cancel: 'Cancel', saving: 'Saving…', removing: 'Deleting…',
      ...
      removeTitle: (modelKey) => `Delete pricing override for ${modelKey}?`, removeDescription: 'The model will use built-in pricing again when available.', actionsAria: 'Pricing override actions', noCache: 'Not set',
    },
    tables: {
      ...
      pricingHeaders: ['Provider', 'Model', 'Input / 1M', 'Output / 1M', 'Cache read / 1M', 'Cache write / 1M', 'Actions'], requestHeaders: ['Time', 'Type', 'Target', 'Session', 'Tokens', 'Cost', 'Latency', 'Status'],
